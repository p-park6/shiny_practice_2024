...
server <- function(input, output) {
  ...
  trout_filtered_df <- reactive ({
    ...
    validate(
      need(length(input$channel_type_input) > 0, "Please select at least one channel type to visualze data for."),
  ...
  #alt text
  alt = "This is my alt text and this will be updated soon"
  
  ) #END trout scatterplot
  
  
  #filter penguin island data ----
  penguin_filtered_df <- reactive({
  ...
  #hisogram island data ----
  output$penguins_histogram_output <- renderPlot({
    
    validate(
      need(length(input$island_input) > 0, "Please select at least one island to visualize the data")
    ) #END validate_penguin
    
    ggplot(na.omit(penguin_filtered_df()), aes(x = flipper_length_mm, fill = species)) +
